...
class SharedMemoryPubSubBase(PubSub[str, Any]):
    ...
    def publish(self, topic: str, message: bytes) -> None:
        ...
        # Synchronous local delivery (lossless for same-process subscribers)
        ...
        for cb in list(st.subs):
            try:
                ...
                pass
        # Let fanout suppress this same frame when it comes back off the channel
        ...
        with self._lock:
            ...
            if prefer in ("cuda", "auto"):
                try:
                    ...
                    logger.info("SharedMemory falling back to CPU backend")
            else:
                ch = CPU_IPC_Factory.create(shape, dtype=dtype)
