        ...
        def publish_lowstate(data):
            ...
            if "data" in data and "bms_state" in data["data"]:
                ...
                battery = BatteryState(
                    ...
                    temperature=float(
                        sum(bms.get("bq_ntc", [0, 0])) / 2.0
                    ),  # Average battery temps
                    power_supply_status=BatteryState.POWER_SUPPLY_STATUS_DISCHARGING
                    if bms.get("current", 0) < 0
...
class UnitreeGo2(Robot):
    ...
    def _deploy_connection(self):
        ...
        self.connection.battery_state.transport = core.LCMTransport(
            "/go2/battery_state", BatteryState
        )
