...
/// Rust equivalent of Python dimos/protocol/pubsub/lcmpubsub.py:LCMTransport
/// 1:1 API intent: publish/subscribe/close.
/// Divergence: Internally uses in-process bus as placeholder instead of real LCM.
#[pyclass]
pub struct LCMTransport {
    // In real implementation, hold LCM context/handle.
    closed: bool,
}
...
impl LCMTransport {
    ...
    pub fn new(_url: Option<String>) -> Self {
        // Divergence note: Python may accept connection strings; we store none.
        Self { closed: false }
    }
    ...
    /// Subscribe to a topic, returning a subscription handle.
    pub fn subscribe(&self, topic: String) -> PyResult<LCMSubscription> {
        if self.closed { return Err(pyo3::exceptions::PyRuntimeError::new_err("transport closed")); }
        Ok(LCMSubscription { topic: topic.clone(), inner: InprocSubscriber::new(topic), closed: false })
    }

    pub fn close(&mut self) { self.closed = true; }
}

/// Subscription handle backing async recv.
#[pyclass]
