...
def main():
    ...
    try:
        ...
        def navigate_to_object():
            try:
                result = robot_skills.call(
                    "Navigate", robot=robot, query=object_name, timeout=timeout
                )
                print(f"Navigation result: {result}")
            except Exception as e:
                print(f"Error during navigation: {e}")

        navigate_thread = threading.Thread(target=navigate_to_object, daemon=True)
        navigate_thread.start()

        print(
            f"Navigating to {object_name} with desired distance {distance}m and timeout {timeout}s..."
        )
